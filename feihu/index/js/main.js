	...
	function addmain(kk,json){
		 ...
		 $.getJSON(json,function(obj){
			   ...
			   for(var i=0;i<arr.length;i++){
			   		var obj=arr[i]
			   		if(i==0){
			   		   $("<a href=#><img src="+obj.img+"></a>").appendTo( $(mainll))
			   		}else if(i==1)
			   		{ $("<li><a href=#><img src="+obj.img+"></a></li>").appendTo( $(ul1))}
			   		else if(i==2)
			   		{{ $("<li><a href=#><img src="+obj.img+"></a></li>").appendTo( $(ul1))}}
			   		else{
			   		var dl=$("<li><dl></dl></li>").appendTo($(ul2))
			   		 $(" <dt><a href=#><img src="+obj.img+"></a></dt>").appendTo( $(dl))
			   		  $(" <dd><a href=# class='mainname'>"+obj.name+"</a></dd>").appendTo( $(dl))
			   		 $("<a href=#><b class=mainprice>"+obj.unit+obj.prices+"</b></a>").appendTo( $(dl))	    
			   		}
		        }//for
